...
var thermistorArray = dataplotArray = helpArray = []; /** Plots graph */
	...
	return {
		...
		link: function(scope, element, attrs) {
			...
			function handleComplete() {
				...
				/** Text box loading */
				setText("voltmeterTxt", 580, 344, "0.00", "black", 1.3);
				setText("voltmeterSymbol", 612, 353, "mA", "black", 0.7);
				setText("batteryTxt", 76, 372, "0.10", "black", 1);
				setText("batterySymbol", 112, 372, "V", "black", 1);
				setText("thermistorTxt", 470, 430, "25 ℃", "black", 1.1);
				...
				dataplotArray.push({x: (0),y: (0)});
				makeGraph(); /** Graph plotting function */						
				thermistor_stage.getChildByName("battery_downarrow").on("click", function() {
					 /** Battery down arrow click event */
					voltage_increase = false;					
					voltageChange(scope);/** Decreasing the voltage while clicking down arrow */
				});
				thermistor_stage.getChildByName("battery_uparrow").on("click", function() {
					 /** Battery up arrow click event */
...
function voltageSliderFN(scope) {
	/**voltage will display on the battery and current can br calculated by I = V/R */
	clearInterval(temp_timer);
...
function startExperiment(scope) {
	if (wire_numbers == 4) {// if the wire number reaches 4. connections complete.
		if (!poweron_flag) {
			...
			thermistor_stage.getChildByName("voltmeterTxt").text = "0.00";
...
function currentCalculation() {
	...
	var current = (voltage_float / resistance_value);
	/**convert the current into milliAmpere*/
	current_ma = (current * 1000);
	if (poweron_flag) { /**ammeter reading change when power on.*/
		thermistor_stage.getChildByName("voltmeterTxt").text = current_ma.toFixed(2);
	}
